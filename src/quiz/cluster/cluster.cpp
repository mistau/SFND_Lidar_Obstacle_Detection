...
void cluster_rec(int index, const std::vector<std::vector<float>>& points, std::vector<int> cluster, std::vector<bool>& processed, KdTree* tree, float distanceTol)
{
    ...
    for(int id : nearest)
        {
            if(!processed[id])
                cluster_rec(id, points, cluster, processed, tree, distanceTol);
        }
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	std::vector<std::vector<int>> clusters;
        std::vector<bool> processed(points.size(), false);   // bit vector to mark if a point was processed already

        for(int i=0; i<points.size(); ++i){
            if(!processed[i]){
                std::vector<int> cluster;
                cluster_rec(i, points, cluster, processed, tree, distanceTol);
